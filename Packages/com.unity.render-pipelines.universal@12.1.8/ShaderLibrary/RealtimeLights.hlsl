...
float DistanceAttenuation(float distanceSqr, half2 distanceAttenuation)
{
    ...
    half smoothFactor = saturate(distanceSqr * distanceAttenuationFloat.x + distanceAttenuationFloat.y);
#endif

    return distanceAttenuation.y * smoothFactor;
}

float DistanceAttenuation(float distanceSqr, float dist, half2 distanceAttenuation)
{
    // good enough for me
    // We use a shared distance attenuation for additional directional and puctual lights
    // for directional lights attenuation will be 1
    // float lightAtten = rcp(0.3 * distanceSqr);
    float2 distanceAttenuationFloat = float2(distanceAttenuation);
#if SHADER_HINT_NICE_QUALITY
    // Use the smoothing factor also used in the Unity lightmapper.
    half factor = half(distanceSqr * distanceAttenuationFloat.x);
...
Light GetAdditionalPerObjectLight(int perObjectLightIndex, float3 positionWS)
{
    ...
    float attenuation = half(DistanceAttenuation(distanceSqr, distance( lightPositionWS, positionWS ), distanceAndSpotAttenuation.xy) * AngleAttenuation(spotDirection.xyz, lightDirection, distanceAndSpotAttenuation.zw));
    // float attenuation = DistanceAttenuation(distanceSqr, distanceAndSpotAttenuation.xy);
    // float dist = distance( lightPositionWS, positionWS );
    // float attenuation = clamp( rcp( 1 + 0.3 * dist), 0, 1 );
