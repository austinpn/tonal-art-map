...
namespace Scripts{
public class TonalArtMapGeneratorShaderBased {
		...
		// manually
		public TonalArtMapGeneratorShaderBased(
				int potSize,
				Texture strokeTex,
				Color sourceColor,
				int toneLevels,
				...
				float maxLength = 0.4f)
		{
			...
			this.reducerMat = this.reducerMat = new Material(Shader.Find("Custom/Reducer"));
			this.size = 1 << potSize;
			this.strokeTex = strokeTex;
            this.sourceColor = sourceColor;
            this.sourceColorValue = ColorValue(sourceColor);
			Debug.Log("Source Value: " + sourceColorValue);
			...
			this.blitShader = Shader.Find("Custom/GeneratorStrokeShader");
			this.height = height;
			...
			for (int tone = 0; tone < toneLevels; tone++) {
				for (int mip = 0; mip < mipLevels; mip++) {
					...
					MainTextures[tone, mip] = new Texture2D(mipSize, mipSize, TextureFormat.RGB24, false);
		...
		Texture2D IntializeTexture(int size) {
