...
Shader "Custom/TonalArtMapFragSixBlendShader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f input) : SV_Target
            {
                ...
                float4 hatch_col1 = ( float4 )0;
                float4 hatch_col2 = ( float4 )0;
                hatch_col1.x = whiten( SAMPLE_TEXTURE2D_ARRAY_LOD(_TonalArtMap, sampler_TonalArtMap, input.uv, 1, mipLevel) );
                hatch_col1.y = whiten( SAMPLE_TEXTURE2D_ARRAY_LOD(_TonalArtMap, sampler_TonalArtMap, input.uv, 2, mipLevel) );
                hatch_col1.z = whiten( SAMPLE_TEXTURE2D_ARRAY_LOD(_TonalArtMap, sampler_TonalArtMap, input.uv, 3, mipLevel) );
                
                hatch_col2.x = whiten( SAMPLE_TEXTURE2D_ARRAY_LOD(_TonalArtMap, sampler_TonalArtMap, input.uv, 4, mipLevel) );
                // hatch_col2.y = whiten( SAMPLE_TEXTURE2D_ARRAY_LOD(_TonalArtMap, sampler_TonalArtMap, input.uv, 5, mipLevel) );
                // hatch_col2.z = whiten( SAMPLE_TEXTURE2D_ARRAY_LOD(_TonalArtMap, sampler_TonalArtMap, input.uv, 6, mipLevel) );
                // hatch_col2.z = whiten( SAMPLE_TEXTURE2D_ARRAY_LOD(_TonalArtMap, sampler_TonalArtMap, input.uv, 7, mipLevel) );
                
                float blended = dot( col, hatch_col1 ) + dot( col, hatch_col2 );
                // float blended = hatch_col1.gb;
